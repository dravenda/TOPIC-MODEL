...
#Free tutorials:
#1. https://www.learnbyexample.org/r-introduction/  (Recommended)
#2. https://www.datamentor.io/r-programming/
...
#The data types can be logical, integer, double, character, complex or raw (bytes)
...
y <- c("unversitat", "de", "barcelona") #Atomic vector of type character (always in quotations)
...
typeof(z) #all vector elements are converted (coerced) to character
...
anyNA(x1) #at least one NA

#NA cannot be used in comparisons: In other packages, a “missing” value is assigned an extreme numeric value

a <- 0/0 #NaN (not a number)

is.na(a)  #NaN included in NA
...
format(today, format="%B %d %Y") #set the format of dates
...
b <- cbind(c(1,2,3),c(4,5,6)); b    #generic function

c <- rbind(c(1,2,3),c(4,5,6)); c    #generic function
...
#List (a type of vector non-atomic) is a data structure having components of mixed data types
...
mydat <- edit(data.frame()) #to enter data manually (unlikely to use)
...
#the sintax of ifelse statement is:
...
##For Loops repeats a chunk of code many times, once for each element in a set of input.
##For loops provide a way to tell R, “Do this for every value of that.”
##In R syntax, this looks like:
for (value in vector) {
  statements
}
...
v <- LETTERS[1:4] #print first four letters
for ( i in v) {
  print(i)
}


chars <- vector(length = 4) #empty vector of length 4

#fill vector chars with strings:
...
for (x in 0:4) print(x) # for loop without curly braces, print the numbers 0 to 4
...
#Break in for Loop is used to exit the loop immediately
