...
?filter #filter function is included in dplyr package included in tidyverse package
...
sum(is.na(df_ar$numpage)) #display number of NA

df_ar$numpage[is.na(df_ar$numpage)] <- mean(df_ar$numpage, na.rm = TRUE) #replace missing with mean

df_ar$numauth <- str_count(df_ar$`Author(s) ID`, ";") #create variable number of authors by counting ;
...
#Three corpora to be created: titles, abstracts and keywords
...
iconv("això és la solució a la qüestió", to='ASCII//TRANSLIT')  #example of function "iconv" run
...
View(dfm_title@docvars)  #access to slot with @

#Lemmatize dfm (optional), to be done for all dfm matrices (we will not do it):
...
#Weigh dfm: 0.4 dfm_title, 0.4 dfm_keyw, 0.2 dfm_abstr (titles and keywords greater weight than abstracts)
...
#Remove words appearing in more than 60% of documents (not interested in recurring topics):

dim(dfm_group) #check number of columns before trimming
dfm_group <- dfm_trim(dfm_group, max_docfreq = 0.60, docfreq_type = "prop")
dim(dfm_group) #check number of columns after trimming
...
topics <- c(20:40)  #vector including topic numbers to test

#the for loop may take about 20 minutes:
for (i in topics){
